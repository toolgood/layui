layui.define(['laytpl', 'layer'], function (exports) {
    ...
    job.prototype.pullData = function () {
        ...
        data[_config.request.keywords] = _config.filter;
        data = $.extend(data, _config.params)
        if (_config.useRsa) {
            rsa = $.createPasswrod();
            data = $.encrypt(data, rsa.key, rsa.rsaKey);
        }

        $.ajax($.extend({
            type: _config.method || "POST",
            url: _config.url,
            ...
            success: function (resp) {
                if (_config.useRsa) {
                    if (resp.ciphertext != undefined && resp.ciphertext.length > 2) {
                        resp.data = JSON.parse($.decrypt(resp.ciphertext, rsa.key));
                    }
                }
                console.log(resp.data);
                if (resp.state == "SUCCESS") {
                    _config.data = resp.data;
    ...
    system.init = function (e, c) {
        var c = c || {},  _elems = $(e ? 'input[lay-filter="' + e + '"]' : 'input[' + filter + ']');
