layui.define(['layer', 'laypage', 'table'], function (exports) {
    ...
    var treetable = {
        ...
        reload: function (p1, p2) {
            if (typeof (p1) == "string") {
                param = paramTables[p1];
                if (param == undefined) {
                    return;
                }
                param = JSON.parse(param);
                if (p2) {
                    param = $.extend({}, param, p2)
                    paramTables[p1] = JSON.stringify(param);
                }
            } else {
                param = $.extend({}, p1, p2)
                var name = param.elem.replace("#", "");
                paramTables[name] = JSON.stringify(param);
            }

            param.where = param.where || {};
            var data = param.where || {};

            if (param.page && param.request) {
                if (param.request.pageName == undefined && data[param.request.pageName] == undefined) {
                    data[param.request.pageName] = 1;
                    param.where[param.request.pageName] = 1;
                }
                if (param.request.limitName == undefined && data[param.request.limitName] == undefined) {
